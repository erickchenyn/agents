...
![](https://pbs.twimg.com/media/HDNlwyUbQAEWZLm?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNmq2FbkAAPRmY?format=jpg&name=large)

只强化其中一层，系统就会失衡，CLAUDE.md 写太长，上下文先污染自己了；工具堆太多了，选择就搞不清楚了；subagents 开得到处都是，状态就漂移了；验证这步跳过了，出了问题根本不知道是哪里挂的。

![](https://pbs.twimg.com/media/HDNmz0xbUAAK0qh?format=png&name=large)
...
![](https://pbs.twimg.com/media/HDNnGJybQAA7KCR?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNngxyaMAAcgV9?format=jpg&name=large)

## 3. 上下文工程：最重要的系统约束

很多人把上下文当"容量问题"，但卡住的地方通常不是不够长，而是太吵了，有用的信息被大量无关内容淹没了。

![](https://pbs.twimg.com/media/HDNnxhbbQAEmIP5?format=png&name=large)
...
![](https://pbs.twimg.com/media/HDNn8yFbQAIICH4?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNoHhibQAY_flX?format=jpg&name=large)

### 压缩机制的陷阱

默认压缩算法按"可重新读取"判断，早期的 Tool Output 和文件内容会被优先删掉，顺带把架构决策和约束理由也一起扔了。两小时后再改，可能根本不记得两小时前定了什么，莫名其妙的 Bug 就是这么来的。

![](https://pbs.twimg.com/media/HDNoRWTbQAQLNdd?format=png&name=large)
...
![](https://pbs.twimg.com/media/HDNoiGJbQAUjQKF?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNomYAbQAMSRGR?format=jpg&name=large)
...
每个 Skill 都在偷你的上下文空间，每个启用的 Skill，描述符常驻上下文，优化前后差距很大：
...
![](https://pbs.twimg.com/media/HDNp6PubQAQLtnc?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNqC4cbQAAB9VJ?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNqIrfaMAAHaV2?format=jpg&name=large)

说白了，既然你就是要 Claude 停下来问一句，那就直接给它一个专门的工具。加个 flag 或者约定一段输出格式，很多时候它一顺手就略过去了。

![](https://pbs.twimg.com/media/HDNqL8ta4AAcN7Y?format=jpg&name=large)
...
最初用 RAG 向量数据库，虽然快但需要索引、不同环境脆弱，最重要的是 Claude 不喜欢用。改成 Grep 工具让 Claude 自己搜索后，效果显著提升。后来又发现一个顺带的好处：Claude 读 Skill 文件，Skill 文件又引用其他文件，模型会递归读取，按需发现信息，不需要提前塞进去，这个模式后来被叫做"渐进式披露"。
...
![](https://pbs.twimg.com/media/HDNqUs6bQAIoN6w?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNqhp3akAAntdH?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNqpqgbQAE9WB8?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNq7sibQAMI3un?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNrVY3bQAQFm9Z?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNrmnsbsAE7Fti?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNsdBSbAAA_nY5?format=jpg&name=large)
...
![](https://pbs.twimg.com/media/HDNsXW1bAAAYfy2?format=jpg&name=large)
